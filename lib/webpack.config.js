...
const getCacheGroups = require('./package-utils').getCacheGroups
...
const base = ({
  ...
  optimization: {
    splitChunks: {
      chunks: 'all',
      cacheGroups: {
        ...getCacheGroups(packageJson).reduce((blob, name) => {
          blob[name] = {
            test(module) {
              return module.identifier().includes('node_modules') && module.identifier().includes(name)
            },
            name,
            chunks: 'all'
          }
...
const test = (base, { main, packageJson }) =>
  merge.smart(base, {
    ...
    entry: [path.resolve(__dirname, 'uncaught-exception.js')]
      .concat(
        getTestSetupPath(packageJson)
          ? [path.resolve(`${getTestSetupPath(packageJson)}`)]
          : []
      )
      .concat(glob.sync(`${getTestPath(packageJson)}**/*spec.js`).map(resolve))
      .concat(
        glob.sync(`${getTestPath(packageJson)}**/*spec.jsx`).map(resolve)
      ),
    output: {
...
const devServer = ({ auth, target, publicPath }) => ({
  ...
  proxy: ['/admin', '/search', '/services', '/webjars', '/direct', '/custom/direct'].reduce(
    (o, url) => {
      o[url] = {
        auth,
        target,
