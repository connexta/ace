...
const { ContextReplacementPlugin, DefinePlugin, ProgressPlugin } = webpack
...
const getPagesPath = require('./package-utils').getPagesPath
...
function extractHTMLPages(packageJson) {
  let htmlFiles = []
  ...
  const pagesPath = resolve(getPagesPath(packageJson))
  if (existsSync(pagesPath)) {
    htmlFiles.push(
      ...sync(path.join(pagesPath, '**/*.html')).map((file) => ({
        filename: path.basename(file),
        template: resolve(file),
      }))
    )
  }

  if (htmlFiles.length === 0) {
    if (existsSync(resolve('src/main/webapp/index.html'))) {
      htmlFiles.push({
        filename: 'index.html',
        template: resolve('src/main/webapp/index.html'),
      })
    } else if (existsSync(resolve('src/main/webapp/index.js'))) {
      htmlFiles.push({
        filename: 'index.html',
        template: resolve('src/main/webapp/index.js'),
      })
    }
  }
...
const dev = (base, { main, packageJson }) => {
  return merge.smart(base, {
    ...
    plugins: [
      ...
      ...extractHTMLPages(packageJson).map(
        ({ filename, template }) =>
