...
const importUsername = async () => {
  try {
    ...
    return (await import('username')).default
  }
}
...
module.exports = async ({ args, getPackage, getPom }) => {
  ...
  // Replace the existing username import with the new pattern
  const username = await importUsername()
  const currentUser = await username()
